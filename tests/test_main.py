...
class TestMain(absltest.TestCase):
  ...
  RSSI = -75
  BLE_DEVICE_UNKNOWN_MAC = BLEDevice('unknown-mac', THERMOMETERS[0].device_name, None, RSSI)
  BLE_DEVICE = BLEDevice(THERMOMETERS[0].device_mac, THERMOMETERS[0].device_name, None, RSSI)
  ...
  def test_unknownDeviceMac_noPoints(self):
    detection_callback(self.BLE_DEVICE_UNKNOWN_MAC, self.AD_DATA)
  ...
  def test_dataPointValueError_warns(self):
    ...
    self.assertIn('Error when building data point.', [record.message for record in logs.records])
    LineProtocolCache.put.assert_not_called()
